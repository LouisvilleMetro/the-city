        ...
        return request.get(googleApiUrl)
                .then(function (verifiedAddress) {
                    ...
                    if (verifiedAddress.status && verifiedAddress.status == 'OK') {
                        ...
                        response._session.attributes.VerfiedCity = verifiedCity;
                    } else {
                        speechOutput = {
...
function handleAmazonYesIntent(event, response) {
    let streetAddress = event.request.session.attributes.VerifiedStreetAddress;
    let city = event.request.session.attributes.VerifiedCity;
    
    if (streetAddress && streetAddress.value && city && city.value) {
