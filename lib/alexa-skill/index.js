...
function handleProvideAddressIntent(event, response) {
    ...
    if (streetAddress && streetAddress.value) {
        ...
        return new Promise(function (resolve) {
            let speechOutput = {
                speech: `Uhmmm! I didn't quite get that. Please, repeat the street address, number first.`,
                type: AlexaSkill.speechOutputType.PLAIN_TEXT
            };
...
console.log(streetAddress + zipCode.value);
    if (streetAddress && zipCode && zipCode.value) {
        let googleApiUrl = config.googleGeoApi.url + config.googleGeoApi.format + '?address=' + streetAddress;
