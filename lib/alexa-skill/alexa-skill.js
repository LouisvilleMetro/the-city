...
AlexaSkill.prototype.handle = function (event) {
    verifyRequest(event, this._appId);
...
function verifyRequest(event, appID) {
    let providedAppID    = event.session.application.applicationId,
        requestTimestamp = moment(event.request.timestamp),
        now              = moment.utc();

    if (appID && providedAppID !== appID) {
    ...
    //if (!requestTimestamp || Math.abs(now.diff(requestTimestamp, 'seconds')) > 150) {
